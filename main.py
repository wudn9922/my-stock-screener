...
def get_dify_report():
    ...
    # 由於搜尋與 AI 統整需要時間，設定 timeout 避免斷線
    response = requests.post(url, headers=headers, json=data, timeout=120)
    response.raise_for_status()
    
    result = response.json()
    # 提取 Dify 結束節點的 output text
    report = result.get("data", {}).get("outputs", {}).get("text", "")
    return report
...
if __name__ == "__main__":
    try:
        ...
        if report_text:
            ...
            send_line_notify("⚠️ Dify 工作流執行成功，但未返回任何文字。請檢查 Dify 的結束節點設定。")
    except Exception as e:
        error_msg = f"❌ 晨報生成失敗: {str(e)}"
        print(error_msg)
